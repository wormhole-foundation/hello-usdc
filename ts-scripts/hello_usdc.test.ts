...
describe("Hello USDC Integration Tests on Testnet", () => {
    test("Tests the sending of USDC", async () => {
        const arbitraryTokenAmount = ethers.BigNumber.from((new Date().getTime()) % (10 ** 5));

        const USDCSourceChain = IERC20__factory.connect(getChain(sourceChain).USDC, getWallet(sourceChain));

        const USDCAddressOnTargetChain = getChain(targetChain).USDC;
        const USDCTargetChain = IERC20__factory.connect(USDCAddressOnTargetChain, getWallet(targetChain));
        ...
        const walletOriginalBalanceOfUSDC = await USDCTargetChain.balanceOf(walletTargetChainAddress);

        const cost = await sourceHelloUSDCContract.quoteCrossChainDeposit(targetChain);
        ...
        const approveTx = await USDCSourceChain.approve(sourceHelloUSDCContract.address, arbitraryTokenAmount).then(wait);
        console.log(`HelloUSDC contract approved to spend ${ethers.utils.formatEther(arbitraryTokenAmount)} of USDC`)

        console.log(`Sending ${ethers.utils.formatEther(arbitraryTokenAmount)} USDC`);

        const tx = await sourceHelloUSDCContract.sendCrossChainDeposit(targetChain, targetHelloUSDCContract.address, walletTargetChainAddress, arbitraryTokenAmount, {value: cost});
